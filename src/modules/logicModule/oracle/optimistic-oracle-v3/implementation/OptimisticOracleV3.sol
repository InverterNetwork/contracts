...
import
    "../../data-verification-mechanism/interfaces/IdentifierWhitelistInterface.sol";
import
    "../../data-verification-mechanism/interfaces/OracleAncillaryInterface.sol";
    ...
    MultiCaller
{
    ...
    function assertTruth(
...

        {
            ...
            EscalationManagerSettings storage emSettings =
                assertions[assertionId].escalationManagerSettings;
            (
        ...
        nonReentrant
    {
        ...
        require(
            assertion.expirationTime > getCurrentTime(), "Assertion is expired"
        );
        ...
        assertion.currency.safeTransferFrom(
            msg.sender, address(this), assertion.bond
        );

        _oracleRequestPrice(
            assertionId, assertion.identifier, assertion.assertionTime
        );
    ...
    function settleAssertion(bytes32 assertionId) public nonReentrant {
        ...
        if (assertion.disputer == address(0)) {
            ...
            int resolvedPrice = _oracleGetPrice(
                assertionId, assertion.identifier, assertion.assertionTime
            );
            ...
            uint oracleFee = (burnedBondPercentage * assertion.bond) / 1e18;
            uint bondRecipientAmount = assertion.bond * 2 - oracleFee;
            ...
            if (!assertion.escalationManagerSettings.discardOracle) {
                _callbackOnAssertionResolve(
                    assertionId, assertion.settlementResolution
                );
            }
    ...
    function syncUmaParams(bytes32 identifier, address currency) public {
        ...
        cachedIdentifiers[identifier] =
            _getIdentifierWhitelist().isIdentifierSupported(identifier);
        cachedCurrencies[currency].isWhitelisted =
            _getCollateralWhitelist().isOnWhitelist(currency);
        cachedCurrencies[currency].finalFee =
            _getStore().computeFinalFee(currency).rawValue;
    }
    ...
    function _getOracle(bytes32 assertionId)
        internal
        view
        returns (OracleAncillaryInterface)
    {
        if (
    ...
    function _oracleRequestPrice(
        ...
        _getOracle(assertionId).requestPrice(
            identifier, time, _stampAssertion(assertionId)
        );
    }

    // Returns the resolved resolution from the Oracle (UMA DVM or escalation manager).
    function _oracleGetPrice(bytes32 assertionId, bytes32 identifier, uint time)
        internal
        view
        returns (int)
    {
        return _getOracle(assertionId).getPrice(
            identifier, time, _stampAssertion(assertionId)
        );
    }

    // Returns the escalation manager address for the assertion.
    function _getEscalationManager(bytes32 assertionId)
        internal
        view
        returns (address)
    {
        ...
        returns (EscalationManagerInterface.AssertionPolicy memory)
    {
        ...
        if (em == address(0)) {
            return EscalationManagerInterface.AssertionPolicy(
    ...
    function _isDisputeAllowed(bytes32 assertionId)
        internal
        view
        returns (bool)
    {
        ...
        return EscalationManagerInterface(em).isDisputeAllowed(
