...
contract SuperbondEscalationManager is BaseEscalationManager, Ownable {
    ...
    function setSuperbond(uint newSuperbond) public onlyOwner {
        superbond = newSuperbond;
    }

    function setSuperbondCurrency(address newSuperbondCurrency)
        public
        onlyOwner
    {
        ...
        returns (AssertionPolicy memory)
    {
        ...
        bool isSuperbondCurrency =
            address(assertion.currency) == superbondCurrency;
        return AssertionPolicy({
            blockAssertion: false,
            arbitrateViaEscalationManager: isSuperbondCurrency
                ? assertion.bond > superbond
                : false,
            discardOracle: false,
