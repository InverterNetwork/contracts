...
contract DisputeLimitingEscalationManager is BaseEscalationManager, Ownable {
        ...
        onlyOptimisticOracleV3
    {
        ...
        if (
            optimisticOracleV3.getAssertion(assertionId)
                ...
                .assertingCaller == assertingCaller
        ) {
    ...
    function assertionResolvedCallback(bytes32 assertionId, bool)
        public
        override
        onlyOptimisticOracleV3
    {
        // Remove assertion block if the disputed assertion was resolved.
        if (assertionId == disputedAssertionId) {
            disputedAssertionId = bytes32(0);
        }
    }

    function _checkIfShouldBlockAssertion(bytes32 assertionId)
        internal
        view
        returns (bool)
    {
        OptimisticOracleV3Interface.Assertion memory assertion =
            optimisticOracleV3.getAssertion(assertionId);
        if (
            assertion.escalationManagerSettings.assertingCaller
                != assertingCaller
        ) return true; // Only allow assertions through linked client contract.
