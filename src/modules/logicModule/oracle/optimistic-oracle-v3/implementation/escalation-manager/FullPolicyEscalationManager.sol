...
contract FullPolicyEscalationManager is BaseEscalationManager, Ownable {
    ...
    event DisputeCallerWhitelistSet(
        address indexed disputeCaller, bool whitelisted
    );

    event AssertingCallerWhitelistSet(
        address indexed assertingCaller, bool whitelisted
    );
    ...
    function getAssertionPolicy(bytes32 assertionId)
        ...
        view
        override
        returns (AssertionPolicy memory)
    {
     ...
     */
    function getPrice(bytes32 identifier, uint time, bytes memory ancillaryData)
        public
        view
        override
        returns (int)
    {
        ...
        require(
            arbitrationResolutions[requestId].valueSet,
            "Arbitration resolution not set"
        );
     ...
     */
    function isDisputeAllowed(bytes32 assertionId, address disputeCaller)
        public
        view
        override
        returns (bool)
    ...
    function configureEscalationManager(
        ...
        require(
            !_blockByAsserter || _blockByAssertingCaller,
    ...
    function setArbitrationResolution(
        ...
        arbitrationResolutions[requestId] =
            ArbitrationResolution(true, arbitrationResolution);
        emit ArbitrationResolutionSet(
            identifier, time, ancillaryData, arbitrationResolution
        );
    }
     ...
     */
    function setDisputeCallerInWhitelist(address disputeCaller, bool value)
        public
        onlyOwner
    {
     ...
     */
    function setWhitelistedAsserters(address asserter, bool value)
        public
        onlyOwner
    {
    ...
    function _checkIfAssertionBlocked(bytes32 assertionId)
        internal
        view
        returns (bool)
    {
        ...
        return (
            blockByAssertingCaller
                && !whitelistedAssertingCallers[assertion
