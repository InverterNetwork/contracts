...
contract PredictionMarket is OptimisticOracleV3CallbackRecipientInterface {
    ...
    struct Market {
        ...
        uint reward; // Reward available for asserting true market outcome.
        uint requiredBond; // Expected bond to assert market outcome (OOv3 can require higher bond).
    ...
    event TokensCreated(
        bytes32 indexed marketId, address indexed account, uint tokensCreated
    );
    event TokensRedeemed(
        bytes32 indexed marketId, address indexed account, uint tokensRedeemed
    );
    event TokensSettled(
        bytes32 indexed marketId,
    ...
    function initializeMarket(
        ...
        new ExpandedERC20(string(abi.encodePacked(outcome1, " Token")), "O1T", 18);
        ExpandedIERC20 outcome2Token =
        new ExpandedERC20(string(abi.encodePacked(outcome2, " Token")), "O2T", 18);
        ...
        returns (bytes32 assertionId)
    {
        ...
        uint minimumBond = oo.getMinimumBond(address(currency)); // OOv3 might require higher bond.
        uint bond = market.requiredBond > minimumBond
            ? market.requiredBond
            : minimumBond;
        ...
        assertedMarkets[assertionId] =
            AssertedMarket({asserter: msg.sender, marketId: marketId});
        ...
        if (assertedTruthfully) {
            ...
            if (market.reward > 0) {
                currency.safeTransfer(
                    assertedMarkets[assertionId].asserter, market.reward
                );
            }
    ...
    function createOutcomeTokens(bytes32 marketId, uint tokensToCreate)
        public
    {
        Market storage market = markets[marketId];
        require(
    ...
    function redeemOutcomeTokens(bytes32 marketId, uint tokensToRedeem)
        public
    {
        Market storage market = markets[marketId];
        require(
    ...
    // as currency payout.
    function settleOutcomeTokens(bytes32 marketId)
        public
        returns (uint payout)
    {
        Market storage market = markets[marketId];
        ...
        uint outcome1Balance = market.outcome1Token.balanceOf(msg.sender);
        uint outcome2Balance = market.outcome2Token.balanceOf(msg.sender);
        ...
        emit TokensSettled(
            marketId, msg.sender, payout, outcome1Balance, outcome2Balance
        );
    }
    ...
    function _composeClaim(string memory outcome, bytes memory description)
        internal
        view
    ...
    function _assertTruthWithDefaults(bytes memory claim, uint bond)
        internal
        returns (bytes32 assertionId)
    {
