...
import {IBondingCurveFundingManagerBase} from
    "@bondingCurve/IBondingCurveFundingManagerBase.sol";
import {IRedeemingBondingCurveFundingManagerBase} from
    "@bondingCurve/IRedeemingBondingCurveFundingManagerBase.sol";
    ...
    Module
{
    ...
    IToposBondingCurveFundingManager public fundingManager;
    ...
    */
    ...
    function init2(IOrchestrator, bytes memory dependencyData)
        external
        override(Module)
        initializer2
    {
        ...
        fundingManager = IToposBondingCurveFundingManager(_fundingManager);
    }
    ...
    function processRepayment(
        ...
        for (uint8 _i = 0; _i < repayerContributions.length; _i++) {
            ...
            if (address(this) == repayerContributions[_i].addr) {
                ...
                IRepayer(repayerContributions[_i].addr).transferRepayment(
                    _to, repayerContributions[_i].amount
                );
            }
        }
    ...
    /// @inheritdoc ILiquidityVaultController
    function setRepayers(IRepayer[] calldata _repayers)
        external
        onlyModuleRole(LIQUIDITY_VAULT_CONTROLLER_MANAGER_ROLE)
    {
        ...
        onlyModuleRole(LIQUIDITY_VAULT_CONTROLLER_MANAGER_ROLE)
    {
        ...
        if (
            ...
            if (coverageSurplusPercent >= insuranceTolerance) {
                ...
                ).sell(amountCollateralAssetRedeemed, 0);
            }
            ...
            // insurance
            amountIssuanceAssetMinted = IBondingCurveFundingManagerBase(
                address(fundingManager)
            ...
            IBondingCurveFundingManagerBase(address(fundingManager)).buy(
        ...
        )
    {
        ...
        if (
            ...
            uint issuanceAssetForCollateral = IBondingCurveFundingManagerBase(
                address(fundingManager)
            ).calculatePurchaseReturn(additionalCollateralRequired);
        ...
        )
    {
        ...
        IRepayer[] memory _repayers =
            new IRepayer[](repayers.length + _additionalRepayers.length);
        uint8 i = 0;
        ...
        LibLiquidityVaultStructs.AddressAmount[] memory _repayerContributions =
            new LibLiquidityVaultStructs.AddressAmount[](_repayers.length);
        ...
        for (uint8 k = 0; k < _repayers.length; k++) {
            ...
            if ((_repayableAmount + totalRepayableAmount) >= _totalRepaymentDue)
            {
                ...
                _repayerContributions[k] = LibLiquidityVaultStructs
                    .AddressAmount(address(_repayers[k]), _diff);
                ...
                _repayerContributions[k] = LibLiquidityVaultStructs
                    .AddressAmount(address(_repayers[k]), _repayableAmount);
    ...
    function _issuanceToCollateralBalance() internal view returns (uint) {
        // Get issuance asset balance
        ...
        return IRedeemingBondingCurveFundingManagerBase(address(fundingManager))
    ...
    function _redeemIssuanceAndTransferCollateral(
        ...
        IRedeemingBondingCurveFundingManagerBase(address(fundingManager)).sell(
    ...
    function _adjustCollateralForSaleFeeCost(
        ...
        uint saleFeePercentage = FixedPointMathLib.fdiv(
            fundingManager.getSaleFeeForAmount(_issuanceAssetAmount),
