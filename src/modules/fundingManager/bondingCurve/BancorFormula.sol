...
import {IBancorFormula} from
    "src/modules/fundingManager/bondingCurve/IBancorFormula.sol";
import {BancorUtils} from
    "src/modules/fundingManager/bondingCurve/BancorUtils.sol";
import {SafeMath} from "src/modules/fundingManager/bondingCurve/SafeMath.sol";

/**
 * @notice This contract was forked from the Aragon Fundraising repository.
 * https://github.com/AragonBlack/fundraising/blob/5ad1332955bab9d36cfad345ae92b7ad7dc0bdbe/apps/bancor-formula/contracts/BancorFormula.sol
 */
contract BancorFormula is IBancorFormula, BancorUtils {
    ...
    function calculatePurchaseReturn(
        ...
        uint result;
        uint8 precision;
        uint baseN = _depositAmount.add(_connectorBalance);
        (result, precision) =
            power(baseN, _connectorBalance, _connectorWeight, MAX_WEIGHT);
        uint temp = _supply.mul(result) >> precision;
        return temp - _supply;
    }
    ...
    function calculateSaleReturn(
        ...
        (result, precision) =
            power(_supply, baseD, MAX_WEIGHT, _connectorWeight);
        uint temp1 = _connectorBalance.mul(result);
        uint temp2 = _connectorBalance << precision;
    ...
    function calculateCrossConnectorReturn(
        ...
        // validate input
        require(
            _fromConnectorBalance > 0 && _fromConnectorWeight > 0
                && _fromConnectorWeight <= MAX_WEIGHT && _toConnectorBalance > 0
                && _toConnectorWeight > 0 && _toConnectorWeight <= MAX_WEIGHT
        );

        // special case for equal weights
        if (_fromConnectorWeight == _toConnectorWeight) {
            return _toConnectorBalance.mul(_amount)
                / _fromConnectorBalance.add(_amount);
        }

        uint result;
        uint8 precision;
        uint baseN = _fromConnectorBalance.add(_amount);
        ...
        uint temp1 = _toConnectorBalance.mul(result);
        uint temp2 = _toConnectorBalance << precision;
        return (temp1 - temp2) / result;
    }
     ...
     */
    function generalExp(uint _x, uint8 _precision)
        internal
        pure
        returns (uint)
    {
    ...
    function optimalLog(uint x) internal pure returns (uint) {
        ...
        z = y = x - FIXED_1;
        w = y * y / FIXED_1;
        res += z * (0x100000000000000000000000000000000 - y)
            / 0x100000000000000000000000000000000;
        z = z * w / FIXED_1; // add y^01 / 01 - y^02 / 02
        res += z * (0x0aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa - y)
            / 0x200000000000000000000000000000000;
        z = z * w / FIXED_1; // add y^03 / 03 - y^04 / 04
        res += z * (0x099999999999999999999999999999999 - y)
            / 0x300000000000000000000000000000000;
        z = z * w / FIXED_1; // add y^05 / 05 - y^06 / 06
        res += z * (0x092492492492492492492492492492492 - y)
            / 0x400000000000000000000000000000000;
        z = z * w / FIXED_1; // add y^07 / 07 - y^08 / 08
        res += z * (0x08e38e38e38e38e38e38e38e38e38e38e - y)
            / 0x500000000000000000000000000000000;
        z = z * w / FIXED_1; // add y^09 / 09 - y^10 / 10
        res += z * (0x08ba2e8ba2e8ba2e8ba2e8ba2e8ba2e8b - y)
            / 0x600000000000000000000000000000000;
        z = z * w / FIXED_1; // add y^11 / 11 - y^12 / 12
        res += z * (0x089d89d89d89d89d89d89d89d89d89d89 - y)
    ...
    function optimalExp(uint x) internal pure returns (uint) {
        ...
        if ((x & 0x010000000000000000000000000000000) != 0) {
            res = res * 0x1c3d6a24ed82218787d624d3e5eba95f9
                / 0x18ebef9eac820ae8682b9793ac6d1e776;
        } // multiply by e^2^(-3)
        if ((x & 0x020000000000000000000000000000000) != 0) {
            res = res * 0x18ebef9eac820ae8682b9793ac6d1e778
                / 0x1368b2fc6f9609fe7aceb46aa619baed4;
        } // multiply by e^2^(-2)
        if ((x & 0x040000000000000000000000000000000) != 0) {
            res = res * 0x1368b2fc6f9609fe7aceb46aa619baed5
                / 0x0bc5ab1b16779be3575bd8f0520a9f21f;
        } // multiply by e^2^(-1)
        if ((x & 0x080000000000000000000000000000000) != 0) {
            res = res * 0x0bc5ab1b16779be3575bd8f0520a9f21e
        ...
        if ((x & 0x200000000000000000000000000000000) != 0) {
            res = res * 0x00960aadc109e7a3bf4578099615711d7
                / 0x0002bf84208204f5977f9a8cf01fdce3d;
        } // multiply by e^2^(+2)
        if ((x & 0x400000000000000000000000000000000) != 0) {
            res = res * 0x0002bf84208204f5977f9a8cf01fdc307
