...
interface IVirtualIssuanceSupplyBase {
    ...
    error VirtualIssuanceSupply__VirtualSupplyCannotBeZero();

    /// @notice Subtracting would result in an underflow.
    error VirtualIssuanceSupply__SubtractResultsInUnderflow();

    /// @notice Adding would result in and overflow.
    error VirtualIssuanceSupply_AddResultsInOverflow();
