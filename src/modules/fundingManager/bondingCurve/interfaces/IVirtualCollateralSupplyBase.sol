...
interface IVirtualCollateralSupplyBase {
    //--------------------------------------------------------------------------
    // Errors

    /// @notice The virtual supply cannot be zero.
    error VirtualCollateralSupplyBase__VirtualSupplyCannotBeZero();

    // @notice Subtracting would result in an underflow.
    error VirtualCollateralSupplyBase__SubtractResultsInUnderflow();

    /// @notice Adding would result in and overflow.
    error VirtualCollateralSupplyBase_AddResultsInOverflow();
