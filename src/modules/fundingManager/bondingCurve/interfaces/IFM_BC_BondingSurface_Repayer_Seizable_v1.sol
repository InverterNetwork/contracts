...
interface IFM_BC_BondingSurface_Repayer_Seizable_v1 {
    ...
    error FM_BC_BondingSurface_Repayer_Seizable_v1__InvalidLiquidityPool(address caller);
    ...
    error FM_BC_BondingSurface_Repayer_Seizable_v1__InvalidFeePercentage(uint fee);

    /// @notice Amount exeeds the seizable amount, defined by a percentage of total collateral
    error FM_BC_BondingSurface_Repayer_Seizable_v1__InvalidSeizeAmount(uint amount);

    /// @notice Timestamp is still in the future, so a seize is not allowed
    error FM_BC_BondingSurface_Repayer_Seizable_v1__SeizeTimeout(uint allowedTimestamp);
