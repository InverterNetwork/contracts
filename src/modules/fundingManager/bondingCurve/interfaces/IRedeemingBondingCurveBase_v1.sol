...
interface IRedeemingBondingCurveBase_v1 {
    ...
    error Module__RedeemingBondingCurveBase_v1__SellingAlreadyOpen();

    /// @notice Function is only callable if selling is not already closed.
    error Module__RedeemingBondingCurveBase_v1__SellingAlreadyClosed();

    /// @notice Percentage amount is bigger than 100%, i.e. 10_000
    error Module__RedeemingBondingCurveBase_v1__InvalidFeePercentage();

    /// @notice Deposit amount has to be larger than zero
    error Module__RedeemingBondingCurveBase_v1__InvalidDepositAmount();

    /// @notice Selling functionalities are set to closed
    error Module__RedeemingBondingCurveBase_v1__SellingFunctionaltiesClosed();

    /// @notice Not enough collateral in contract for redemption
    error Module__RedeemingBondingCurveBase_v1__InsufficientCollateralForRedemption(
    );

    /// @notice Actual redeem amount is lower than the minimum acceptable amount
    error Module__RedeemingBondingCurveBase_v1__InsufficientOutputAmount();
