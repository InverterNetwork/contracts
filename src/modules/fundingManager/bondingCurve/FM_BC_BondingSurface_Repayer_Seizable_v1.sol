    ...
    RedeemingBondingCurveBase
{
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return interfaceId == type(IFM_BC_BondingSurface_Repayer_Seizable_v1).interfaceId
    ...
    modifier onlyLiquidityPool() {
        if (_msgSender() != address(liquidityPool)) {
            revert FM_BC_BondingSurface_Repayer_Seizable_v1__InvalidLiquidityPool(
                _msgSender()
            );
        }
        _;
    ...
    function seize(uint _amount) public onlyModuleRole(COVER_MANAGER_ROLE) {
        uint s = seizable();
        if (_amount > s) {
            revert FM_BC_BondingSurface_Repayer_Seizable_v1__InvalidSeizeAmount(s);
        }
    ...
    {
        if (_seize > MAX_SEIZE) {
            revert FM_BC_BondingSurface_Repayer_Seizable_v1__InvalidSeize(_seize);
        }
        currentSeize = _seize;
    }
        ...
        onlyModuleRole(COVER_MANAGER_ROLE)
    {
        if (_fee > MAX_SELL_FEE) {
            revert FM_BC_BondingSurface_Repayer_Seizable_v1__InvalidFeePercentage(_fee);
        }
        _setSellFee(_fee);
    }
    ...
    function _setBasePriceMultiplier(uint _newBasePriceMultiplier) internal {
        ...
        emit BasePriceMultiplierChanged(basePriceMultiplier, _newBasePriceMultiplier);
