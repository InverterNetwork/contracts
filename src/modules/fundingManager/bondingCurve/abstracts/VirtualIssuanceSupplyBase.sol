...
abstract contract VirtualIssuanceSupplyBase is IVirtualIssuanceSupplyBase, ERC165 {
    ...
    function _addVirtualTokenAmount(uint _amount) internal {
        if (_amount > (MAX_UINT - virtualIssuanceSupply)) {
            revert VirtualIssuanceSupply_AddResultsInOverflow();
        }
        unchecked {
    ...
    function _subVirtualTokenAmount(uint _amount) internal {
        if (_amount > virtualIssuanceSupply) {
            revert VirtualIssuanceSupply__SubtractResultsInUnderflow();
        }

        if (_amount == virtualIssuanceSupply) {
            revert VirtualIssuanceSupply__VirtualSupplyCannotBeZero();
        }
        unchecked {
            virtualIssuanceSupply -= _amount;
        }
