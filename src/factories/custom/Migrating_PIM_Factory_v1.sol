...
contract Migrating_PIM_Factory_v1 is ERC2771Context, IMigrating_PIM_Factory_v1 {
    ...
    function setIssuanceFeeMultiplier(
        ...
        emit IMigrating_PIM_Factory_v1.IssuanceFeeMultiplierChanged(
            oldMultiplier,
            _issuanceFeeMultiplier
        );
    }

    function setMutableInitialMintAmount(
        uint _mutableInitialMintAmount
    ) external onlyAdmin {
    ...
    function buyFor(
        ...
        uint validAmountIn = _checkBuyExceedsThreshold(
            fundingManager,
            amountIn
        );
    ...
    function sellTo(
        ...
        IERC20Issuance_v1 issuanceToken = IERC20Issuance_v1(
            fm.getIssuanceToken()
        );
    ...
    function _checkBuyExceedsThreshold(
        address fundingManager,
        uint amountIn
    ) internal view returns (uint validAmountIn) {
        PIM memory pim = pims[fundingManager];
    ...
    function _graduate(address fundingManager) internal {
        ...
        LM_PC_PaymentRouter_v1 paymentRouter = LM_PC_PaymentRouter_v1(
            pim.paymentRouter
        );
        FM_BC_Restricted_Bancor_Redeeming_VirtualSupply_v1 fm = FM_BC_Restricted_Bancor_Redeeming_VirtualSupply_v1(
                fundingManager
        ...
        uint adminCollateralFee = (collateralLiquidity *
            collateralFeeMultiplier) / FEE_DENOMINATOR;
        ...
        uint adminIssuanceFee = (issuanceLiquidity * issuanceFeeMultiplier) /
            FEE_DENOMINATOR;
    ...
    function _deployIssuanceToken(
        ...
        if (
            !isImmutable && admin != address(0) && mutableInitialMintAmount > 0
        ) {
        ...
        )
    {
        ...
        fundingManagerConfig.configData = abi.encode(
            address(issuanceToken),
            bcProperties,
            collateralToken
        );
