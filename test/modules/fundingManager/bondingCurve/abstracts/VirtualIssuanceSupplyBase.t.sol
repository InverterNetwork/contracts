...
import {VirtualIssuanceSupplyBaseMock} from
    "test/modules/fundingManager/bondingCurve/mocks/VirtualIssuanceSupplyBaseMock.sol";
...
contract VirtualIssuanceSupplyBaseTest is Test {
    ...
    function testAddTokenAmount(uint amount) external {
        ...
        // Test event
        vm.expectEmit(true, true, false, false, address(virtualIssuanceSupplyBase));
    ...
    function testAddTokenAmountFails(uint amount) external {
        ...
        vm.expectRevert(
            IVirtualIssuanceSupplyBase.VirtualIssuanceSupply_AddResultsInOverflow.selector
        );
        virtualIssuanceSupplyBase.addVirtualTokenAmount(amount);

        assertEq(
            virtualIssuanceSupplyBase.getVirtualIssuanceSupply(), (INITIAL_SUPPLY)
        );
    }
    ...
    function testSubTokenAmountFails(uint amount) external {
        ...
        vm.expectRevert(
            IVirtualIssuanceSupplyBase
                .VirtualIssuanceSupply__SubtractResultsInUnderflow
                .selector
        );
        virtualIssuanceSupplyBase.subVirtualTokenAmount(amount);
    }
