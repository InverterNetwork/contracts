...
import {ERC20IssuanceRebasing_v1} from
    "src/external/token/ERC20Issuance_RebasingMock_v1.sol";

import {SafeERC20} from "@oz/token/ERC20/utils/SafeERC20.sol";

contract LM_PC_StakeToInvest_Prototype is E2ETest {
    ...
    address yieldTreasury = makeAddr("Yieldtreasury");
    ...
    ERC20IssuanceRebasing_v1 stakingToken = new ERC20IssuanceRebasing_v1(
        "Staking Mock", "S_MOCK", 18, type(uint).max, address(this)
    );

    // Default values for the streaming payments
    uint defaultStart = block.timestamp;
    uint defaultCliff = 1 days;
    uint defaultEnd = block.timestamp + 7 days;
    ...
    function test_e2e_LM_PC_Staking_v1Lifecycle() public {
        ...
        // Set up yieldTreasury
        stakingManager.setTreasuryAddress(yieldTreasury);
        ...
        // 1. We deposit a lot of rewardTokens into the FundingManager
        uint initialDeposit = 1e28;
        rewardToken.mint(address(this), initialDeposit);
        rewardToken.approve(address(fundingManager), initialDeposit);
        fundingManager.deposit(initialDeposit);

        // 2. We mint stakingTokens to the stakers
        stakingToken.mint(address(staker1), 10e18);
        stakingToken.mint(address(staker2), 20e18);
        stakingToken.mint(address(staker3), 50e18);
        stakingToken.mint(address(staker4), 100e18);

        // 3. Approve that stakingManager can use stakingToken
        vm.prank(staker1);
        stakingToken.approve(address(stakingManager), 10e18);
        vm.prank(staker2);
        stakingToken.approve(address(stakingManager), 20e18);
        vm.prank(staker3);
        stakingToken.approve(address(stakingManager), 50e18);
        vm.prank(staker4);
        stakingToken.approve(address(stakingManager), 100e18);

        // 4. Set up reward period 1

        stakingManager.setRewards(amount1, duration1);

        // 5. All users stake the same amount
        vm.prank(staker1);
        stakingManager.stake(10e18);
        ...
        // 7. check yield generated balances

        assertEq(
            stakingToken.balanceOf(address(stakingManager)),
            (40e18 + 40 * stakingToken.REBASE_AMOUNT())
        );

        // 8. Claim the yield

        stakingManager.withdrawYield();

        assertEq(
            stakingToken.balanceOf(yieldTreasury),
            (40 * stakingToken.REBASE_AMOUNT())
        );
        assertEq(stakingToken.balanceOf(address(stakingManager)), 40e18);
